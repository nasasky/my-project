...
class WechatAuth {
  ...
  getCode() {
    ...
    let authParams = `?appid=${this.config.appid}&redirect_uri=${
      this.config.redirect_uri
    }&response_type=${this.config.responseType}&scope=${
      this.config.scope
    }#wechat_redirect`;
  ...
  getCodeCallback(next, code, scene_id) {
    ...
    axios
      ...
      .then(res => {
        ...
        if (success) {
          console.log("router");
          next(this.config.main_uri); // 获取access_toeken成功
