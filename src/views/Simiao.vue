<template>
  <div class="hc">
    <div class="login-container">
      ...
      <van-row class="box" style="width:100%;">
        <van-button
          type="primary"
          size="small"
          class="login-btn"
        ...
        >{{isLogin ? '登录' : '注册并且登录'}}</van-button>
      </van-row>
    </div>
  </div>
</template>
<script>
import { simiaologin } from "@/api/pc-simiao";
export default {
  data() {
    return {
      username: "hu",
    ...
    simiaologin() {
      ...
      simiaologin(data)
        .then(response => {
          ...
          console.log("返回code", response.data.code);

          if (response.data.code == 203) {
            this.$router.replace("/adminx");
            // console.log(2222);
...
.login-container {
  ...
  -webkit-box-shadow: 0 0 30px #cac6c6;
